...
function init_fvm(
    ...
    elseif KS.set.space[3:4] == "1f"
        ...
        for i in eachindex(ctr)
            ...
            ctr[i] = ControlVolume(funcar(w), funcar(prim), funcar(f), 1; auxiliary = KS.set.hasForce)
        end
    ...
    elseif KS.set.space[3:4] == "2f"
        ...
        for i in eachindex(ctr)
            ...
            ctr[i] = ControlVolume(funcar(w), funcar(prim), funcar(h), funcar(b), 1; auxiliary = KS.set.hasForce)
        end
...
function init_fvm(
    ...
    elseif KS.set.space[3:4] == "1f"
        ...
        for j in axes(ctr, 2), i in axes(ctr, 1)
            ...
            ctr[i, j] = ControlVolume(funcar(w), funcar(prim), funcar(h), 2; auxiliary = KS.set.hasForce)
        end
    ...
    elseif KS.set.space[3:4] == "2f"
        ...
        for j in axes(ctr, 2), i in axes(ctr, 1)
            ...
            ctr[i, j] = ControlVolume(funcar(w), funcar(prim), funcar(h), funcar(b), 2; auxiliary = KS.set.hasForce)
        end
