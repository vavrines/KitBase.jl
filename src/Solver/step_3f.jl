...
function step!(
    ...
    w_old = ws.w_old
    MHT, MBT, MRT, MHR, MBR, MRR = 
        ws.MHT, ws.MBT, ws.MRT, ws.MHR, ws.MBR, ws.MRR
    SHT, SBT, SRT, SHR, SBR, SRR = 
        ws.SHT, ws.SBT, ws.SRT, ws.SHR, ws.SBR, ws.SRR
    MH, MB, MR = ws.MH, ws.MB, ws.MR
...
function step!(
    ...
    w_old, prim_old, mw, mprim, tau, H0, H1, H2 = 
        ws.w_old, ws.prim_old, ws.mw, ws.mprim, ws.tau, ws.H0, ws.H1, ws.H2
    ...
    # flux -> w^{n+1}
    @. cell.w += (faceL.fw - faceR.fw) / dx
    mixture_conserve_prim!(cell.prim, cell.w, KS.gas.γ)
    ...
    if isMHD == false
        ...
        mixture_prim_conserve!(mw, mprim, KS.gas.γ)
        for k in axes(cell.w, 2)
            @. cell.w[:, k] += (mw[:, k] - w_old[:, k]) * dt / tau[k]
        end
        mixture_conserve_prim!(cell.prim, cell.w, KS.gas.γ)
    ...
    mixture_prim_conserve!(cell.w, cell.prim, KS.gas.γ)
