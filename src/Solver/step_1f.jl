...
function step!(
    ...
    w_old, M, S = ws.w_old, ws.MH, ws.SH
...
function step!(
    ...
    if collision == :shakhov
        q = heat_flux(f, prim, uVelo, vVelo, wVelo, weights)
        maxwellian!(M, uVelo, vVelo, wVelo, prim)
        shakhov!(S, uVelo, vVelo, wVelo, M_old, q, prim, Pr, K)
    else
        fill!(S, zero(eltype(S)))
    end
...
1D1F3V fast spectral method with workspace
"""
function step!(
    w::T3,
    prim::T3,
...
function step!(
    ...
    w_old, MH, SH = ws.w_old, ws.MH, ws.SH
    ...
    if collision == :shakhov
        ...
        maxwellian!(MH, u, v, prim)
        shakhov!(SH, u, v, MH_old, q, prim, Pr)
    else
        fill!(SH, zero(eltype(SH)))
    end
...
2D1F2V with workspace @ triangle
"""
function step!(
    w::T1,
    prim::T1,
    ...
    if collision == :shakhov
        ...
        shakhov!(S, u, v, M_old, q, prim, Pr, K)
    else
        fill!(S, zero(eltype(S)))
    end
...
2D1F3V fast spectral method with workspace
"""
function step!(
    w::T3,
    prim::T3,
