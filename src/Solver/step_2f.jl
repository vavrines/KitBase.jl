...
function step!(
    ...
    w_old, MH, MB, SH, SB = ws.w_old, ws.MH, ws.MB, ws.SH, ws.SB
    ...
    if collision == :shakhov
        q = heat_flux(h, b, prim, u, weights)
        maxwellian!(MH, u, prim)
        MB .= MH .* K ./ (2.0 * prim[end])
        shakhov!(SH, SB, u, MH, MB, q, prim, Pr, K)
    else
        fill!(SH, zero(eltype(SH)))
        fill!(SB, zero(eltype(SB)))
    end
...
function step!(
    ...
    w_old, prim_old, mw, mprim, tau, H, B = 
        ws.w_old, ws.prim_old, ws.mw, ws.mprim, ws.tau, ws.H0, ws.H1
...
function step!(
    ...
    if collision == :shakhov
        ...
        MB .= MH .* K ./ (2.0 * prim[end])
        shakhov!(SH, SB, u, v, MH_old, MB_old, q, prim, Pr, K)
    else
        fill!(SH, zero(eltype(SH)))
        fill!(SB, zero(eltype(SB)))
    end
...
2D2F2V with workspace @ triangle
"""
function step!(
    w::T1,
    prim::T1,
