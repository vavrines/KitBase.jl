...
function update_transport!(
    ...
    for i=1:np
        ...
        if tc > dt 
            ...
            if ptc[i].x > KS.pSpace.x0 && ptc[i].x < KS.pSpace.x1
                ...
                if ptc_temp[no_particle_temp].v[1]<0.0
                    ptc_temp[no_particle_temp].tb = (ctr[cell_no_temp].x-
                        0.5*ctr[cell_no_temp].dx-ptc_temp[no_particle_temp].x)/ptc_temp[no_particle_temp].v[1]
        ...
        elseif tc > ptc[i].tb
            ...
            if ptc[i].x > KS.pSpace.x0 && ptc[i].x < KS.pSpace.x1
                ...
                ctr[cell_no_temp].wg[3]+=0.5*ptc[i].m*sum(ptc[i].v.^2)/ctr[cell_no_temp].dx
            end
        end
    end
    ...
    for i=1:no_particle_temp
        ...
        ctr[particle_cell_temp].w[2]+=ptc_temp[i].m*ptc_temp[i].v[1]/ctr[particle_cell_temp].dx
        ctr[particle_cell_temp].w[3]+=0.5*ptc_temp[i].m*sum(ptc_temp[i].v.^2)/ctr[particle_cell_temp].dx
    end
...
function update_collision!(
    ...
    for i in 1:KS.pSpace.nx
        ...
        for j in 1:no_particle_cell_temp
            ...
            @. ptc_temp[no_particle_temp].v= sqrt(-log(rd1) / ctr[i].prim[end]) * sin(2.0 * π * rd2)
            ...
            if ptc_temp[no_particle_temp].v[1]<0
                ptc_temp[no_particle_temp].tb=(ctr[i].x-
                    0.5*ctr[i].dx-ptc_temp[no_particle_temp].x)/ptc_temp[no_particle_temp].v[1]
    ...
    if bc == :fix    
        ...
        for i = 1:ng
            ...
            for j=1:no_particle_cell_temp
                ...
                ptc_temp[no_particle_temp].mass=KS.gas.m
                @. ptc_temp[no_particle_temp].v= sqrt(-log(rd1) / ctr[idx].prim[end]) * sin(2.0 * π * rd2)
                ...
                if ptc_temp[no_particle_temp].v[1]<0
                    ptc_temp[no_particle_temp].tb=(ctr[idx].x-
                        0.5*ctr[idx].dx-ptc_temp[no_particle_temp].x)/ptc_temp[no_particle_temp].v[1]
                elseif ptc_temp[no_particle_temp].v[1]>0
                    ptc_temp[no_particle_temp].tb=(ctr[idx].x+
                        0.5*ctr[idx].dx-ptc_temp[no_particle_temp].x)/ptc_temp[no_particle_temp].v[1]
                else
