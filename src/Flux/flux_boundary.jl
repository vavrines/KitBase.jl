...
function flux_boundary_maxwell!(fw::AV, a1, a2, a3, a4, a5, a6, a7, a8, a9, rot)
    if length(fw) == 3
        ...
        fw[3] =
            (0.5 * discrete_moments(hWall .* u .^ 2, u, ω, 1) +
             0.5 * discrete_moments(bWall, u, ω, 1)) * dt
    ...
    elseif length(fw) == 5
        ...
        SF = sum(ω .* u .* f .* (1.0 .- δ))
        SG =
            (bc[end] / π)^1.5 * sum(ω .* u .*
                exp.(-bc[end] .*
                     ((u .- bc[2]) .^ 2 .+ (v .- bc[3]) .^ 2 .+ (w .- bc[4]) .^ 2),) .* δ,)
        prim = [-SF / SG; bc[2:end]]
...
function flux_boundary_maxwell!(fw::AM, fh, fb, bc, h, b, u, ω, inK, dt, rot)
    ...
    SG = [
        (bc[end, j] / π)^0.5 *
        sum(ω[:, j] .* u[:, j] .* exp.(-bc[end, j] .* (u[:, j] .- bc[2, j]) .^ 2) .*
            δ[:, j],) for j in axes(h, 2)
    ]
    ...
    for j in axes(fw, 2)
        ...
        fw[3, j] =
            (0.5 * discrete_moments(hWall[:, j] .* u[:, j] .^ 2, u[:, j], ω[:, j], 1) +
             0.5 * discrete_moments(bWall[:, j], u[:, j], ω[:, j], 1)) * dt
    end
