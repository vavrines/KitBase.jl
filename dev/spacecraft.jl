...
[
    @show point_distance(points[i, j, end, :], set.xc) for i in axes(points, 1),
    j in axes(points, 2)
]
...
ni, nj, nk = size(points)[1:3] .- 1
x0, y0, z0 =
    minimum(points[:, :, :, 1]), minimum(points[:, :, :, 2]), minimum(points[:, :, :, 3])
x1, y1, z1 =
    maximum(points[:, :, :, 1]), maximum(points[:, :, :, 2]), maximum(points[:, :, :, 3])
x = zeros(ni, nj, nk)
y = zero(x)
z = zero(x)
dx = zero(x)
